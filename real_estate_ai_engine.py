...
# Configuration
API_KEY = "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"
...
class GroqAIService:
    ...
    def get_available_models(self) -> List[str]:
        try:
            ...
            self.available_models = [m["id"] for m in models_data.get("data", [])]
    ...
    def switch_to_next_model(self) -> Optional[str]:
        ...
        for available_model in self.available_models:
            if available_model not in PREFERRED_MODELS:
                logger.warning(f"⚠️ Using emergency fallback model: {available_model}")
    ...
    def send_chat_completion(self, messages: List[Dict[str, str]], model: str = None) -> Dict[str, Any]:
        if not model:
            ...
            # Reset model index for fresh start
            self.current_model_index = 0
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                logger.info(f"📤 Sending request to model: {model} (attempt {attempt + 1})")
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"⏳ Rate limited by model {model}. Switching to next model immediately...")
                    
                    new_model = self.switch_to_next_model()
                    if new_model:
                        model = new_model
                        logger.info(f"🔄 Switched to {model} due to rate limit")
                        continue
                    else:
                        logger.error("❌ No more models available after rate limit")
                        break
